...
class pathens(object):
    ...
    def make_clusters(self, threshold=0.5, pickleit=True):
    ...
    def simple_cluster(self, threshold=0.5):
        reprclusters = {}
        reprscores = {}
        clusters = []
        totalclnum = 0
...
def cluster_pathways(cutoff, outpickle, fileprefix, totalruns=0, thresholdcl=0.5, runvector=[]):
    starttime = time.time()
    pE = pathens()
...
def convergence_test(cutoff, outpickle, fileprefix, totalruns, maxgroupsize, numiters=50, thresholdcl=0.5, outfile='clusters.txt'):
    pE = pathens()
    pE.set_fileprefix(fileprefix)
    pE.get_uniq_paths(range(1, totalruns+1), cutoff)
    print (pE.get_uniq_paths(range(1,totalruns+1),cutoff))
    pE.make_clusters(threshold=thresholdcl)
    ...
    with open(outpickle, 'w') as handle:
        pickle.dump(pE, handle)
    #pE.plot_convergence_by_runs(figurename='hist.png')
    #pE.evaluate_accuracy_precision()

    #print (pE.get_ranks_by_position())
